                        ...
                        bn_i2s_gemv_fn gemv, int n_threads)
{
    ...
    for (int i = 0; i < nt; i++) {
        ...
        tasks[i].gemv      = gemv;
        pthread_create(&threads[i], NULL, bn_gemv_thread, &tasks[i]);
    }
    for (int i = 0; i < nt; i++) {
                           ...
                           int n_threads)
{
    ...
    for (int i = 0; i < nt; i++) {
        ...
        pthread_create(&threads[i], NULL, bn_f32mv_thread, &tasks[i]);
    }
...
bitnet_model_t *bitnet_model_load(const char *path) {
    ...
    if (te->type == BN_GGML_TYPE_F32) {
    ...
    } else if (te->type == BN_GGML_TYPE_F16) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            memcpy(&m->token_embd[i], &f, 4);
        }
    }

    bn_gguf_tensor_t *on = bn_gguf_find_tensor(g, "output_norm.weight");
    if (on) m->output_norm = (float *)on->data;
    ...
    for (int l = 0; l < m->n_layer; l++) {
        ...
        #define LOAD_F32(dst, fmt, ...) do { \
            snprintf(name, sizeof(name), fmt, __VA_ARGS__); \
            bn_gguf_tensor_t *_t = bn_gguf_find_tensor(g, name); \
            if (_t) dst = (float *)_t->data; \
        } while(0)
        ...
        #define LOAD_I2S(dst, wscale_dst, scale_dst, fmt, nr, nc, ...) do { \
            ...
            if (_t) { \
                dst = (uint8_t *)_t->data; \
                wscale_dst = bn_get_i2s_weight_scale(_t->data, nr, nc); \
            } \
            snprintf(name, sizeof(name), fmt ".scale", __VA_ARGS__); \
            bn_gguf_tensor_t *_s = bn_gguf_find_tensor(g, name); \
            if (_s) scale_dst = *(float *)_s->data; \
        } while(0)

        LOAD_F32(m->layers[l].attn_norm, "blk.%d.attn_norm.weight", l);
        LOAD_F32(m->layers[l].attn_sub_norm, "blk.%d.attn_sub_norm.weight", l);
        LOAD_F32(m->layers[l].ffn_norm, "blk.%d.ffn_norm.weight", l);
        LOAD_F32(m->layers[l].ffn_sub_norm, "blk.%d.ffn_sub_norm.weight", l);
